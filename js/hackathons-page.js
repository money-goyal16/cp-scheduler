...
import { isLoggedIn, login, signup, logout, getUser } from './auth.js';
...
function setupForm() {
  ...
  btn?.addEventListener('click', async () => {
    if (!isLoggedIn()) {
      showToast('Please log in first to add events.', 'error');
      return;
    }
    ...
    try {
      await saveEvent({ name, organizer, startDate, endDate, url, type, location, prize, description: desc });
      clearForm();
      await renderAll();
...
// ── Auth UI (simple login/signup modal) ────────────────────────────
function setupAuthUI() {
  const headerRight = document.querySelector('header .container');
  if (!headerRight) return;

  const authBox = document.createElement('div');
  authBox.id = 'auth-box';
  authBox.style.cssText = 'display:flex;align-items:center;gap:10px;margin-left:12px;';

  if (isLoggedIn()) {
    const user = getUser();
    authBox.innerHTML = `
      <span style="font-family:var(--font-mono);font-size:0.78rem;color:var(--text-muted)">Hi, ${user?.name || 'User'}</span>
      <button id="logout-btn" class="filter-btn">Logout</button>`;
    headerRight.appendChild(authBox);
    document.getElementById('logout-btn').addEventListener('click', logout);
  } else {
    authBox.innerHTML = `<button id="login-open-btn" class="filter-btn">Login / Signup</button>`;
    headerRight.appendChild(authBox);
...
function openAuthModal() {
  ...
  modal.innerHTML = `
    <div style="background:var(--bg-card,#1a1a1a);padding:28px;border-radius:12px;width:320px;font-family:var(--font-mono);">
      <h3 style="margin-bottom:16px;color:var(--text,#fff)">Login</h3>
      <input id="auth-name" placeholder="Name (signup only)" style="width:100%;margin-bottom:8px;padding:8px;" />
      <input id="auth-email" placeholder="Email" style="width:100%;margin-bottom:8px;padding:8px;" />
      <input id="auth-password" type="password" placeholder="Password" style="width:100%;margin-bottom:12px;padding:8px;" />
      <div style="display:flex;gap:8px;">
        <button id="auth-login-btn" class="filter-btn" style="flex:1;">Login</button>
        <button id="auth-signup-btn" class="filter-btn" style="flex:1;">Signup</button>
      </div>
      <button id="auth-close-btn" style="margin-top:12px;background:none;border:none;color:var(--text-muted);cursor:pointer;">Close</button>
    </div>`;
